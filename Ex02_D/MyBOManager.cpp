...
void MyBOManager::DisplaySphere(int a_nIndex, vector3 a_v3Color)
{
	...
	if (a_nIndex >= 0)
	{
		if (v3Color != vector3(-2.0f, -2.0f, -2.0f))
		{
		...
		{
			if (m_llCollidingIndices[a_nIndex].size() > 0)
				m_lObject[a_nIndex]->DisplaySphere(RERED);
		}
	}
	else //all objects sent
	{
		...
		else
		{
			...
			{
				if (m_llCollidingIndices[nObject].size() > 0)
					m_lObject[nObject]->DisplaySphere(RERED);
			}
		}
	}
}
...
void MyBOManager::DisplayOriented(int a_nIndex, vector3 a_v3Color)
{
	...
	if (a_nIndex >= 0)
	{
		...
		{
			if (m_llCollidingIndices[a_nIndex].size() > 0)
				m_lObject[a_nIndex]->DisplayOriented(RERED);
		}
	}
	else //all objects sent
	{
		...
		else
		{
			...
			{
				if (m_llCollidingIndices[nObject].size() > 0)
					m_lObject[nObject]->DisplayOriented(RERED);
			}
		}
	}
}
...
void MyBOManager::DisplayReAlligned(int a_nIndex, vector3 a_v3Color)
{
	...
	if (a_nIndex >= 0)
	{
		...
		{
			if (m_llCollidingIndices[a_nIndex].size() > 0)
				m_lObject[a_nIndex]->DisplayReAlligned(RERED);
		}
	}
	else //all objects sent
	{
		...
		else
		{
			...
			{
				if (m_llCollidingIndices[nObject].size() > 0)
					m_lObject[nObject]->DisplayReAlligned(RERED);
			}
		}
	}
}
void MyBOManager::ClearCollidingLists(void)
{
	for (uint nObject = 0; nObject < m_nObjectCount; nObject++)
	{
		m_llCollidingIndices[nObject].clear();
...
void MyBOManager::MarkCollisionInList(uint a_nIndexA, uint a_nIndexB)
{
	if (a_nIndexA >= m_nObjectCount || a_nIndexB >= m_nObjectCount)
		return;
	m_llCollidingIndices[a_nIndexA].push_back(a_nIndexB);
	m_llCollidingIndices[a_nIndexB].push_back(a_nIndexA);
}
void MyBOManager::CheckCollisionBetween(uint a_nIndexA, uint a_nIndexB)
{
	if (m_lObject[a_nIndexA]->IsColliding(m_lObject[a_nIndexB]))
	{
		m_llCollidingIndices[a_nIndexA].push_back(a_nIndexB);
		m_llCollidingIndices[a_nIndexB].push_back(a_nIndexA);
	}
