...
#pragma region Other Actions
	...
	ON_KEY_PRESS_RELEASE(F, bFPSControl = !bFPSControl, m_pCameraMngr->SetFPS(bFPSControl));
	bool bScreenShot = false;
	ON_KEY_PRESS_RELEASE(G, NULL, bScreenShot = true);
	if (bScreenShot)
	{
		TextureManagerSingleton* pTextureMngr = TextureManagerSingleton::GetInstance();
		pTextureMngr->SaveTexture("Data\\Screenshots\\" + m_pSystem->GetAppName());
	}
#pragma endregion
}
